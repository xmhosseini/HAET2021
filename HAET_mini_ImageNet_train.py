...
with open('mini-imagenet-cache-train.pkl', 'rb') as f:
    data = pickle.load(f)
train_test = data['image_data'].reshape([64, 600, 84, 84, 3])
x_train = train_test[0:10, 0:500].reshape([5000, 84, 84, 3])
x_test  = train_test[0:10, 500:600].reshape([1000, 84, 84, 3])
y_train = np.zeros((10,500,1))
y_test = np.zeros((10,100,1))
for i in range (10):
    y_train[i]=i
    y_test[i]=i
...
xtr2 = np.zeros((5000,32,32,3))
xts2 = np.zeros((1000,32,32,3))
for i in range(x_train.shape[0]):
  xtr2[i] = cv2.resize(x_train[i], (32, 32), interpolation=cv2.INTER_LINEAR)
for i in range(x_test.shape[0]):
  xts2[i] = cv2.resize(x_test[i], (32, 32), interpolation=cv2.INTER_LINEAR)
x_train = xtr2
x_test = xts2
xtr2=0
xts2=0
